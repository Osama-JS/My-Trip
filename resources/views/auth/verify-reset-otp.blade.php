...
<head>
    ...
    <title>{{ __('Verify Reset Code') }} - {{ config('app.name', 'Fly Vio') }}</title>
...
<div class="otp-page-wrapper">
    <div class="otp-card">
        ...
        <div class="otp-header">
            ...
            <p>{{ __('Please enter the 6-digit code sent to') }} <br> <span class="email-highlight">{{ $email }}</span></p>
        </div>
        ...
        <form method="POST" action="{{ route('password.verify-otp') }}" id="otpForm">
            ...
            <button type="submit" class="btn-primary">
                {{ __('Verify Code') }} <i class="fas fa-chevron-{{ app()->isLocale('ar') ? 'left' : 'right' }} ms-1"></i>
            </button>
        </form>
        ...
        <div style="margin-top: 2rem;">
            <a href="{{ route('password.request') }}" style="color: #64748b; font-size: 0.9rem; text-decoration: none;">
                <i class="fas fa-arrow-{{ app()->isLocale('ar') ? 'right' : 'left' }} me-1"></i> {{ __('Back to Reset Request') }}
            </a>
        </div>
    </div>
</div>
