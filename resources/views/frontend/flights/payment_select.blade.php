...
<div class="pay-page mt-5">
    ...
    <div class="fe-container pay-page__inner">
        ...
        <div class="pay-steps" style="margin-top: 50px;">
            ...
            <div class="pay-steps__item done"><span class="pay-steps__bubble"><i class="fas fa-check"></i></span><span class="pay-steps__label">{{ __('Travellers') }}</span></div>
        ...
        @php
            $flightExpiryTime = null;
            $mBooking = (isset($mainBooking) && $mainBooking) ? $mainBooking : \App\Models\Booking::find($booking->booking_id);
            if ($mBooking) {
                $flightExpiryTime = $mBooking->ticketing_time_limit 
                    ?: ($mBooking->created_at ? $mBooking->created_at->copy()->addMinutes(15) : now()->addMinutes(15));
            } elseif ($booking && $booking->created_at) {
                $flightExpiryTime = $booking->created_at->copy()->addMinutes(15);
            } else {
                $flightExpiryTime = now()->addMinutes(15);
            }

            $remainingSeconds = max(0, (int) now()->diffInSeconds($flightExpiryTime, false));
            $initMinutes = floor($remainingSeconds / 60);
            $initSecs = $remainingSeconds % 60;
            $initialDisplay = sprintf('%02d:%02d', $initMinutes, $initSecs);
        @endphp
        <div class="fe-session-timer-card" id="flightPaymentTimeLimit" data-remaining-seconds="{{ $remainingSeconds }}" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); color: #fff; padding: 16px 24px; border-radius: 18px; margin-bottom: 25px; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.2); border: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <div style="font-size: 14px; font-weight: 800; color: #f8fafc; margin-bottom: 2px;">
                        {{ app()->getLocale() == 'ar' ? 'مهلة الدفع وإتمام الحجز (Page Time Limit)' : 'Page Time Limit: Complete Payment' }}
                    </div>
                    <div style="font-size: 12.5px; color: #94a3b8;">
                        {{ app()->getLocale() == 'ar' ? 'المقاعد والأسعار محجوزة مؤقتاً لجلستك الحالية' : 'Seats and fares are held until the countdown expires' }}
                    </div>
                </div>
            </div>
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div style="background: rgba(239, 68, 68, 0.2); border: 1px solid rgba(239, 68, 68, 0.4); color: #f87171; font-family: monospace; font-size: 1.3rem; font-weight: 900; padding: 6px 16px; border-radius: 10px; letter-spacing: 1px;" id="flightPaymentTimerDisplay">
                    {{ $initialDisplay }}
                </div>
            </div>
        </div>
        ...
        <div class="pay-grid">
            ...
            <div class="pay-grid__left">
                <div class="pay-card">
                    ...
                    <div class="pay-methods">
                        ...
                        <a href="{{ route('payments.web.checkout', ['booking_id' => $booking->booking_id, 'method' => 'tap', 'type' => 'flight']) }}" class="pay-method">
                            <div class="pay-method__left">
                        ...
                        {{-- Apple Pay --}}
                        <!-- <a href="{{ route('payments.web.checkout', ['booking_id' => $booking->booking_id, 'method' => 'apple_pay', 'type' => 'flight']) }}" class="pay-method pay-method--dark">
                            <div class="pay-method__left">
                        ...
                        <div class="pay-mini-grid">
                            ...
                            <a href="{{ route('payments.web.checkout', ['booking_id' => $booking->booking_id, 'method' => 'tamara', 'type' => 'flight']) }}" class="pay-mini">
                            ...
                            <a href="{{ route('payments.web.checkout', ['booking_id' => $booking->booking_id, 'method' => 'tabby', 'type' => 'flight']) }}" class="pay-mini">
            ...
            <div class="pay-grid__right">
                <div class="pay-summary">
                    <div class="pay-summary__tag"><i class="fas fa-plane"></i>  {{ __('Flight Booking') }}</div>
                    ...
                    <div class="pay-summary__route">
                        ...
                        <div class="pay-summary__route-line">
                            ...
                            <div class="pay-summary__route-track"><i class="fas fa-plane"></i></div>
                            <div class="pay-summary__route-dot"></div>
                        </div>
                    ...
                    <div class="pay-summary__dates">
                        <div class="pay-summary__date-box">
                            <span>{{ __('Date') }}</span>
                            <strong>{{ $booking->departure_date->format('d M') }}</strong>
                            <small>{{ $booking->departure_date->format('Y') }}</small>
                        </div>
                        ...
                        <div class="pay-summary__date-box pay-summary__date-box--right">
                            <span>{{ __('Class') }}</span>
                            <strong style="font-size:1rem;">{{ $booking->flight_class }}</strong>
                            <small>&nbsp;</small>
                        </div>
                    </div>
                    ...
                    <div class="pay-summary__pill">
                        ...
                        @if($booking->childs > 0) &nbsp;+&nbsp; {{ $booking->childs }} {{ __('Children') }} @endif
                        @if($booking->infants > 0) &nbsp;+&nbsp; {{ $booking->infants }} {{ __('Infants') }} @endif
                    </div>
...
.pay-steps__item.active .pay-steps__bubble { background: white; border-color: white; color: #0f4c81; box-shadow: 0 0 0 6px rgba(255,255,255,.15); }
...
.pay-card__icon-wrap { width: 52px; height: 52px; flex-shrink: 0; background: linear-gradient(135deg, #0f4c81, #1976d2); border-radius: 16px; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.2rem; box-shadow: 0 8px 20px rgba(15,76,129,.35); }
...
.pay-method:hover { background: white; border-color: #0f4c81; box-shadow: 0 8px 24px rgba(15,76,129,.12); transform: translateY(-3px); }
...
.pay-method:hover .pay-method__arrow { background: #0f4c81; border-color: #0f4c81; color: white; }
...
/* ─── Summary Card ─────── */
.pay-summary { background: rgba(255,255,255,.09); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,.2); border-radius: 28px; padding: 36px; position: sticky; top: 24px; color: white; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
.pay-summary__tag { display: inline-flex; align-items: center; gap: 8px; background: rgba(255,255,255,.12); border: 1px solid rgba(255,255,255,.2); border-radius: 100px; padding: 6px 16px; font-size: .73rem; font-weight: 800; text-transform: uppercase; letter-spacing: .5px; color: rgba(255,255,255,.85); margin-bottom: 22px; }
...
.pay-summary__airport strong { font-size: 2rem; font-weight: 950; color: white; line-height: 1; letter-spacing: -.02em; }
...
document.addEventListener('DOMContentLoaded', function() {
    const timerBox = document.getElementById('flightPaymentTimeLimit');
    const timerDisplay = document.getElementById('flightPaymentTimerDisplay');
    ...
    function updateFlightPaymentTimer() {
        if (remainingSeconds <= 0) {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    text: '{{ app()->getLocale() == "ar" ? "انتهت المهلة المحددة لحجز مقاعد وتأكيد سعر هذه الرحلة. يرجى إعادة البحث." : "The hold period for these flight seats and fares has expired. Please search again." }}',
    ...
    updateFlightPaymentTimer();
    timerInterval = setInterval(updateFlightPaymentTimer, 1000);
});
</script>
