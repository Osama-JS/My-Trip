...
<div class="booking-list-container">
    ...
    <div class="filter-bar">
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
            <a href="{{ route('customer.bookings.trips', request()->except(['status', 'page'])) }}" class="filter-btn {{ !request('status') ? 'active' : '' }}">
                {{ __('All Status') }}
            </a>
            <a href="{{ route('customer.bookings.trips', array_merge(request()->except(['page']), ['status' => 'pending'])) }}" class="filter-btn {{ request('status') === 'pending' ? 'active' : '' }}">
                <i class="fas fa-clock"></i> {{ __('Pending') }}
            </a>
            <a href="{{ route('customer.bookings.trips', array_merge(request()->except(['page']), ['status' => 'confirmed'])) }}" class="filter-btn {{ request('status') === 'confirmed' ? 'active' : '' }}">
    ...
    {{-- Advanced Filters Panel --}}
    <form action="{{ route('customer.bookings.trips') }}" method="GET" id="filterForm">
        @if(request('status'))
        ...
        <div class="advanced-filters-panel" id="advancedFilters" style="{{ (request('search') || request('date_from') || request('date_to')) ? 'display: block;' : '' }}">
            <div class="filters-grid">
                <div class="filter-group">
                    ...
                    <input type="text" name="search" class="filter-input" value="{{ request('search') }}" placeholder="{{ __('Search by Booking ID, Trip Name, Country or City...') }}">
                </div>
                <div class="filter-group">
            ...
            <div class="filter-actions">
                <a href="{{ route('customer.bookings.trips', request('status') ? ['status' => request('status')] : []) }}" class="btn-sm btn-outline" style="width: auto;">
                ...
                <div class="booking-details">
                    ...
                    @if($trip?->fromCity && $trip?->toCity)
                        <div class="ticket-route">
                            <span class="route-city">{{ $trip->fromCity->name }}</span>
                            <span class="route-arrow">
                                <span class="route-line"></span>
                                <i class="fas fa-plane"></i>
                            </span>
                            <span class="route-city">{{ $trip->toCity->name }}</span>
                        </div>
                    @endif
                    ...
                    <div class="booking-meta-row">
                        ...
                        @endif
                        @if($trip?->duration)
                            <span><i class="fas fa-clock"></i> {{ $trip->duration }} {{ __('Days') }}</span>
                        @endif
