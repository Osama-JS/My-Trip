...
<div class="row page-titles">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><a href="javascript:void(0)">{{ __('Flight Bookings') }}</a></li>
    </ol>
</div>
    ...
    <div class="col-xl-3 col-sm-6">
        <div class="kpi-card kpi-card--green">
            ...
            <div class="kpi-info">
                ...
                <span class="kpi-badge kpi-badge--green"><i class="fas fa-circle-check me-1"></i>{{ __('Active') }}</span>
            </div>
        </div>
    </div>
    <div class="col-xl-3 col-sm-6">
        <div class="kpi-card kpi-card--amber">
            ...
            <div class="kpi-info">
                ...
                <span class="kpi-badge kpi-badge--amber"><i class="fas fa-hourglass-half me-1"></i>{{ __('Awaiting') }}</span>
            </div>
        </div>
    </div>
    <div class="col-xl-3 col-sm-6">
        <div class="kpi-card kpi-card--red">
            ...
            <div class="kpi-info">
                ...
                <span class="kpi-badge kpi-badge--red"><i class="fas fa-ban me-1"></i>{{ __('Cancelled') }}</span>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-12">
        <div class="dash-table-card">
            ...
            <div class="subs-card-header">
                <div>
                    <h6 class="dash-chart-title">{{ __('Flight Bookings') }}</h6>
                    <p class="dash-chart-sub">{{ __('All flight reservations and their current status') }}</p>
                </div>
            ...
            <div class="card-body p-0 pt-2">
                <div class="table-responsive subs-table-wrap">
                    <table id="flights-table" class="display subs-datatable" style="min-width: 845px">
                        <thead>
                            <tr>
                                <th>{{ __('ID') }}</th>
                                <th>{{ __('User') }}</th>
...
.kpi-card {
    ...
    background: var(--dash-surface); border-radius: var(--dash-radius);
    padding: 24px; box-shadow: var(--dash-shadow);
...
/* Filter Wrapper (select2 compatible) */
...
.filter-wrapper .select2-container .select2-selection--single {
    height: 38px !important; border-radius: 50px !important;
    border: 1px solid var(--dash-border) !important;
    background: #f8fafc !important; padding-left: 30px !important;
}
.filter-wrapper .select2-container .select2-selection--single .select2-selection__rendered {
    line-height: 38px !important; color: var(--dash-text) !important; font-size: 13px !important; font-weight: 500 !important;
}
...
/* DataTable */
#flights-table_filter { display: none !important; }
...
/* Action Buttons (same as subscribers) */
.act-action-btn {
    display: inline-flex; align-items: center; justify-content: center;
    width: 34px; height: 34px; border-radius: 10px;
    background: rgba(4,23,65,0.07); color: var(--dash-navy);
    text-decoration: none; transition: all 0.2s ease; font-size: 13px;
    border: none;
}
...
$(document).ready(function() {
    const flightsTable = $('#flights-table').DataTable({
        processing: true,
        serverSide: false,
        ...
        columns: [
            ...
            { data: 'amount' },
            {
                ...
                render: function(data) {
                    const map = {
                        confirmed: ['badge-state--green', '{{ __("Confirmed") }}'],
                        pending:   ['badge-state--amber', '{{ __("Pending") }}'],
                        cancelled: ['badge-state--red',   '{{ __("Cancelled") }}'],
                    };
                    const cfg = map[data] || ['badge-state--default', data];
                    return `<span class="badge-state ${cfg[0]}">${cfg[1]}</span>`;
                }
            },
            { data: 'created_at' },
    ...
    // Initialize select2
    $('#filter-status').select2({ minimumResultsForSearch: -1, width: '160px' });

    // Filter logic
    function applyFilters() {
        const statusVal = $('#filter-status').val();
        const searchVal = $('#custom-search').val();
        flightsTable.column(5).search(statusVal ? (statusVal === 'confirmed' ? '{{ __("Confirmed") }}' : statusVal === 'pending' ? '{{ __("Pending") }}' : '{{ __("Cancelled") }}') : '');
        flightsTable.search(searchVal);
        flightsTable.draw();
    }

    $('#filter-status').on('change', applyFilters);
    $('#custom-search').on('keyup', applyFilters);
});
</script>
@endpush
