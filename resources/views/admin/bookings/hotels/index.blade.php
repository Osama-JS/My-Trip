...
<style>
    :root {
        --dash-navy: #041741; --dash-navy-2: #0a2456; --dash-gold: #f5a623;
    ...
    [data-theme-version="dark"] .subs-datatable thead th { background:#161625 !important; }
    ...
    <div class="row my-2">
        <div class="col-xl-3 col-sm-6 my-2">
            <div class="kpi-card kpi-card--blue">
                <div class="kpi-icon-wrap"><i class="fas fa-hotel"></i></div>
                <div class="kpi-info">
        ...
        <div class="col-12">
            <div class="dash-table-card">
                <div class="subs-card-header">
                    <div>
                        <h6 class="dash-chart-title">{{ __('Hotel Bookings') }}</h6>
                        <p class="dash-chart-sub">{{ __('Manage and track all hotel reservations') }}</p>
                    </div>
                ...
                <div class="card-body p-0 pt-2">
                    <div class="table-responsive subs-table-wrap">
                        <table id="hotels-table" class="display subs-datatable" style="min-width: 845px">
                            <thead>
                                <tr>
                                    <th>{{ __('ID') }}</th>
                                    <th>{{ __('User') }}</th>
...
<script>
    $(document).ready(function() {
        $('#hotels-table').DataTable({
            ...
            columns: [
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        let cls = 'badge-state--default';
                        let label = data;
                        if (data === 'confirmed') {
                            cls = 'badge-state--green';
                        } else if (data === 'paid' && row.requires_action) {
                            cls = 'badge-state--amber';
                            label = '⚠️ ' + '{{ __("Paid - Action Needed") }}';
                        } else if (data === 'paid') {
                            cls = 'badge-state--blue';
                        } else if (data === 'pending') {
                            cls = 'badge-state--amber';
                        } else if (data === 'cancelled') {
                            cls = 'badge-state--red';
                        }
                        return `<span class="badge-state ${cls}">${label}</span>`;
                    }
                },
                { data: 'created_at' },
            ...
            createdRow: function(row, data, dataIndex) {
                if (data.requires_action) {
                    $(row).css('border-right', '4px solid #f59e0b');
                }
            },
            language: {
        ...
        // Hide default search, use custom
        $('#hotels-table_filter').hide();
        $('#custom-search').on('keyup', function() {
            $('#hotels-table').DataTable().search(this.value).draw();
        });
    });
</script>
@endpush
